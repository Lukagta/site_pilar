...
const Dashboard = () => {
    ...
    const toggleStatus = async (id: number, currentStatus: boolean) => {
        try {
            await fetch(`${API_URL}/api/admin/doctors/${id}/toggle`, {
                method: 'PATCH'
            });
    ...
    const deleteDoctor = async (id: number) => {
        ...
        try {
            await fetch(`${API_URL}/api/admin/doctors/${id}`, {
                method: 'DELETE'
            });
    ...
    return (
        <div className="p-4 md:p-12">
            <header className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 mb-12">
                <div>
                    <h1 className="font-display text-3xl md:text-4xl font-extrabold text-deep-blue tracking-tighter">Gerenciar Profissionais</h1>
                ...
                <button
                    ...
                    className="bg-primary hover:bg-deep-blue text-white font-bold px-8 py-4 rounded-2xl flex items-center justify-center gap-3 transition-all transform hover:-translate-y-1 shadow-lg shadow-primary/20 group w-full md:w-auto"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mb-12">
                <div className="bg-white p-6 md:p-8 rounded-3xl md:rounded-[2rem] border border-deep-blue/5 shadow-sm">
                    <p className="text-[10px] font-black uppercase tracking-widest text-med-blue/40 mb-2">Total de Médicos</p>
                    <p className="text-3xl md:text-4xl font-display font-black text-deep-blue">{doctors.length}</p>
                </div>
                <div className="bg-white p-6 md:p-8 rounded-3xl md:rounded-[2rem] border border-deep-blue/5 shadow-sm">
                    <p className="text-[10px] font-black uppercase tracking-widest text-med-blue/40 mb-2">Ativos no Site</p>
                    <p className="text-3xl md:text-4xl font-display font-black text-green-500">{doctors.filter(d => d.isActive).length}</p>
                </div>
                <div className="bg-white p-6 md:p-8 rounded-3xl md:rounded-[2rem] border border-deep-blue/5 shadow-sm">
                    <p className="text-[10px] font-black uppercase tracking-widest text-med-blue/40 mb-2">Inativos</p>
                    <p className="text-3xl md:text-4xl font-display font-black text-med-blue/20">{doctors.filter(d => !d.isActive).length}</p>
                </div>
            </div>
